...
function MyEvent() {
        ...
        if (auth.currentUser) {
            ...
            try {
                ...
                for (const doc of querySnapshot.docs) {
                    ...
                    if (eventDetails) {
                        events.push({
                            id: data.eventId, // Add event ID for removal
                            name: data.eventName,
                            image: eventDetails.images[0].url,
                            date: new Date(eventDetails.dates.start.localDate).toLocaleDateString(),
                        });
    ...
    async function handleRemoveEvent(eventId) {
        ...
        try {
            const docRef = doc(database, "RSVP", `${auth.currentUser.uid}_${eventId}`);
            ...
            localStorage.setItem('eventIds', JSON.stringify(updatedEventIds));
